...
# 加载数据
file_path = "Data/stdLSTM.xlsx"
data_22 = pd.read_excel(file_path)
data_23 = pd.read_excel("Data/std23.xlsx")

file_name = os.path.basename(file_path)   # 获取文件名stdXXX.xlsx
model_name = file_name.split('.')[0][3:]  # 提取std后面的部分XXX
...
labels = data.iloc[:, 1].values  # 第2列为烤后氮含量
...
# 初始化模型、损失函数和优化器
model = LSTMModel(input_num, hidden_size, num_layers, output_size)
...
def plot(train_metrics, test_metrics, N_train, N_test):
    ...
    #ax.set_title(r'TSR-LSTM for cNg', title_font, pad=10)
    ax.set_title(r'AE$_{\text{' + model_name + r'}}$-LSTM for cNg', title_font, pad=10)
    ...
    ax.set_xlim((10, 35))
    ax.set_ylim((10, 35))
    ax.set_xticks(np.arange(10, 35.1, step=5))
    ax.set_yticks(np.arange(10, 35.1, step=5))
    ax.set_xlabel("Measured cNg (mg/g)")
    ax.set_ylabel("Predicted cNg (mg/g)")

    # 训练集
    ax.text(13.6, 33.0, r'$R^2_t$={:.2f}'.format(train_metrics['r2']))
    ax.text(10.6, 33.0, r'$N_t$=' + str(N_train))
    ax.text(10.6, 31.5, r'$RMSE_t$={:.2f}mg/g'.format(train_metrics['rmse']))
    ax.text(10.6, 30.0, r'$MAE_t$={:.2f}mg/g'.format(train_metrics['mae']))
    ax.text(10.6, 28.5, r'$MAPE_t$={:.2f}%'.format(train_metrics['mape']))

    # 测试集
    ax.text(30.8, 15.5, r'$R^2_v$={:.2f}'.format(test_metrics['r2']))
    ax.text(28.0, 15.5, r'$N_v$=' + str(N_test))
    ax.text(28.0, 14.0, r'$RMSE_v$={:.2f}mg/g'.format(test_metrics['rmse']))
    ax.text(28.0, 12.5, r'$MAE_v$={:.2f}mg/g'.format(test_metrics['mae']))
    ax.text(28.0, 11.0, r'$MAPE_v$={:.2f}%'.format(test_metrics['mape']))

    plt.tight_layout()
    plt.savefig(f'Figure/{model_name}-LNg.pdf', format='pdf', bbox_inches='tight')
    plt.show()

plot(train_metrics, test_metrics, N_train, N_test)
print(f'图片已保存到 Figure/{model_name}-LNg.pdf')
