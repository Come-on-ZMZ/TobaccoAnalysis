...
# 特征数据转置(样本数, 7, 3), 特征即为通道
feature_new = feature_new.transpose(0, 2, 1)
print("转置后数据性状:", feature_new.shape)  # (105, 7, 3)
...
def std_data(data):
    ...
    for i in range(data.shape[1]):
        data_i = data[:, i, :]
        scaler.fit(data_i)
        data_scaled[:, i, :] = scaler.transform(data_i)
...
class Conv1D_AutoEncoder(nn.Module):
    def __init__(self, input_channels=7, latent_size=7):
        super(Conv1D_AutoEncoder, self).__init__()
        # 编码器
        self.encoder = nn.Sequential(
            nn.Conv1d(in_channels=input_channels, out_channels=28, kernel_size=2, stride=1),
            nn.LeakyReLU(0.1),
            nn.Flatten(),
            nn.Linear(28 * 2, latent_size * 2),
            nn.LeakyReLU(0.1)
        )
        # 解码器
        self.decoder = nn.Sequential(
            nn.Linear(latent_size * 2, 28 * 2),
            nn.LeakyReLU(0.1),
            nn.Unflatten(1, (28, 2)),
            nn.ConvTranspose1d(in_channels=28, out_channels=input_channels, kernel_size=2, stride=1)
        )

    def forward(self, x):
        z = self.encoder(x)
        z_latent = z.view(z.size(0), -1, 2)
...
ax1.set_ylabel('Loss (MSE)')  # MSE Loss
...
plt.title('Loss (MSE) and $R^2$ curves of the AE$_{\\text{Conv1D}}$', title_font, pad=10)
plt.plot([], [], '-', color=color1, label='Train loss (0.03)')
plt.plot([], [], '--', color=color1, label='Test loss (0.09)')
plt.plot([], [], '-', color=color2, label='Train $R^2$ (0.96)')
plt.plot([], [], '--', color=color2, label='Test $R^2$ (0.92)')
plt.legend(loc='center right', borderaxespad=1.0, prop={'size': 11})
plt.tight_layout()
plt.savefig('Figure/AE_C1D.pdf', format='pdf', bbox_inches='tight')
plt.show()
...
def extract_latent(model, data_scaled):
    ...
    return latent_vectors  # (样本数, 7, 2)

# 提取潜在表示
latent_vectors = extract_latent(model, feature_scaled)
print("潜在表示形状:", latent_vectors.shape)  # (105, 7, 2)

# 潜在表示转置回(样本数, 2, 7)
latent_sequences = latent_vectors.transpose(0, 2, 1)
...
# 导出文件
export_data(latent_sequences, 'Data/Latent_Conv1D.xlsx', add_line=add_line)
