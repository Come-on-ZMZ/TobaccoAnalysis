...
def plot(train_metrics, test_metrics, N_train, N_test):
    ...
    #ax.set_title(r'TSR-LSTM for cNt', title_font, pad=10)
    ax.set_title(r'AE$_{\text{'+ model_name + r'}}$-LSTM for cNt', title_font, pad=10)
    ...
    plt.tight_layout()
    plt.savefig(f'Figure/{model_name}-LNt.pdf', format='pdf', bbox_inches='tight')
    plt.show()

plot(train_metrics, test_metrics, N_train, N_test)
print(f'图片已保存到 Figure/{model_name}-LNt.pdf')
